...
// This function generates the invoice PDF
const generateInvoicePDF = async (billingData, address, calculations) => {
  // Extract calculations for use in the invoice
  const {
    total_weight,
  ...
  // Create the product table rows dynamically based on billingData
  const productRows = billingData
    .map(
      ...
      <tr>
        ...
        <td style="border: 1px solid #000; padding: 5px;">${item.price.toFixed(
          2
        )}</td>
        <td style="border: 1px solid #000; padding: 5px;">${item.totalWithOutGST.toFixed(
          2
        )}</td>
      </tr>
    `
  ...
  // Combine all parts of the invoice HTML
  const invoiceHtml = `
    ${INVOICE_TEMPLATE.HEADER}

    <!-- Retailer Information -->
    <div style="padding: 10px 0; border-bottom: 1px solid #000;">
        <p style="margin: 0; font-size: 14px;"><strong>Party:</strong> ${
          address.shopName || "Ram Stationery"
        }</p>
        <p style="margin: 5px 0; font-size: 14px;"><strong>State:</strong> ${
          address.state || "Nagpur"
        }</p>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 14px; text-align: center;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="border: 1px solid #000; padding: 5px;">${total_GST_Amount.toFixed(
                      2
                    )}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td style="border: 1px solid #000; padding: 5px;"><strong>${total_Product_Prize_includingGST.toFixed(
                      2
                    )}</strong></td>
                </tr>
            </tbody>
    ...
    <div style="padding: 10px 0;">
        <p style="margin: 0; font-size: 14px;"><strong>Amount Chargeable (in words):</strong> ${convertToWords(
          total_Product_Prize_includingGST
        )} INR Only</p>
    </div>
    ...
    <div style="padding: 10px 0; border-top: 1px solid #000;">
        ...
        <p style="margin: 0; font-size: 14px;"><strong>Transport Charges (120/kg):</strong> ${total_Transport_expenses_WithoutGST.toFixed(
          2
        )}</p>
        
        <p style="margin: 5px 0; font-size: 14px;"><strong>Transport Tax ${Transport_tax_Rate}:</strong> ${Transport_tax.toFixed(
          2
        )}</p>
        <p style="margin: 5px 0; font-size: 14px;"><strong>Total Transport Expenses:</strong> ${final_Transport_expenses.toFixed(
          2
        )}</p>
    </div>
    ...
    <div style="padding: 10px 0; border-top: 1px solid #000;">
        <table style="width: 100%; border-collapse: collapse; font-size: 14px; text-align: center;">
            ...
            <tbody>
                <tr>
                    <td style="border: 1px solid #000; padding: 5px;">${total_Product_Prize_includingGST.toFixed(
                      2
                    )}</td>
                    
                    <td style="border: 1px solid #000; padding: 5px;">${final_Transport_expenses.toFixed(
                      2
                    )}</td>
                    <td style="border: 1px solid #000; padding: 5px;">${total_Product_Price_includingGST_and_transport_expenses.toFixed(
                      2
                    )}</td>
                </tr>
                <tr>
    ...
    <!-- Tax Amount in Words -->
    <div style="padding: 10px 0;">
        <p style="margin: 0; font-size: 14px;"><strong>Tax Amount (in words):</strong> ${convertToWords(
            total_Product_Price_includingGST_and_transport_expenses
        )} INR Only</p>
    </div>
  ...
  // Convert the HTML to PDF and download it
  try {
